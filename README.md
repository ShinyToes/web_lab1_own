...
- 支持用户输入自定义查询
- 实时显示检索结果（包含文档规范化原文）
- 支持动态切换 TF-IDF 方法（命令：`method log`）
- 提供帮助和配置查看功能（`help`, `config`, `methods`）
- 输入 `quit` 退出程序
...
1. 文档ID: 11095502
   ...
   匹配词项: 1/1
   匹配的词: music
   规范化原文: ali amr music festival addition perform group ali amr experiment...
...
2. 文档ID: 112001832
   ...
   匹配词项: 1/1
   匹配的词: music
   规范化原文: bar new_haven live music every wednesday night...
...
3. 文档ID: 110826922
   ...
   匹配词项: 1/1
   匹配的词: music
   规范化原文: monday morning music meetup look meet parent make friend...
```
...
│   ├── query_1_results.json      # JSON 格式结果（含 normalized_text 字段）
│   ├── query_1_results.txt       # 可读文本格式（含规范化原文）
│   ├── query_2_results.json
│   ├── query_2_results.txt
│   └── ...                       # 其他查询结果
├── retrieval_report.txt           # 详细检索报告
│   # 包含：系统统计、查询统计、词项分析、词项频率分析
└── retrieval_statistics.json     # 统计信息
    # 包含：相似度分布、查询覆盖率、每个查询的详细统计
```

**JSON 结果文件示例** (`query_1_results.json`)：
  ...
  "results": [
    {
      ...
      "match_count": 1,
      "matched_terms": ["music"],
      "normalized_text": "ali amr music festival addition perform group..."
    }
  ]
}
```
...
- **`inverted_index_dir`**: 倒排索引文件目录（包含 `inverted_index.txt`）
- **`documents_dir`**: 规范化文档目录（包含 `*.txt` 文档文件）
- **`output_dir`**: 检索结果输出目录
- **`tfidf_method`**: TF-IDF 计算方法
  ...
  - `binary`: 二元频率方法
- **`top_k`**: 返回的文档数量（默认 10）
- **`min_similarity`**: 最小相似度阈值（默认 0.0，返回所有文档）
- **`test_queries`**: 测试查询列表（10个预定义查询）
...
# 2. 执行测试模式检索（10个预定义查询）
python scripts/run_retrieval.py --mode test --top-k 3

# 3. 查看结果
# - 控制台显示所有查询结果（含规范化原文）
# - output_retrieval/query_results/ 包含每个查询的 JSON 和 TXT 文件
# - output_retrieval/retrieval_report.txt 包含详细统计报告
# - output_retrieval/retrieval_statistics.json 包含统计信息
```
...
1. 文档ID: 11095502
   ...
   匹配词项: 2/2
   匹配的词: music, concert
   规范化原文: ali amr music festival addition perform group...

> methods                            # 查看可用方法
可用的 TF-IDF 方法:
1. standard - 标准 TF-IDF: tf × log(N/df)
2. log - 对数频率: (1 + log(tf)) × log(N/df)
3. augmented - 增强频率: (0.5 + 0.5×tf/max_tf) × log(N/df)
4. binary - 二元频率: (1 if tf>0 else 0) × log(N/df)
...
```bash
# 使用对数频率方法
python scripts/run_retrieval.py --mode test --method log --top-k 5

# 使用增强频率方法（适合长短文档混合场景）
python scripts/run_retrieval.py --mode test --method augmented

# 使用二元方法（适合专有名词检索）
python scripts/run_retrieval.py --mode test --method binary

# 使用自定义配置文件
python scripts/run_retrieval.py --config my_retrieval_config.json
```
