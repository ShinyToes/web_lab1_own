...
# ============ 配置项 ============
DEFAULT_INPUT_DIR = r"D:\2025_2\web\Lab\web_lab1\output\normalized_events"  # 默认输入文件夹路径
DEFAULT_OUTPUT_DIR = r"D:\2025_2\web\Lab\web_lab1\part_3\output"  # 默认输出文件夹路径
INVERTED_INDEX_FILE = "inverted_index.txt"  # 最终倒排索引文件名（文本格式）
INVERTED_INDEX_BIN_FILE = "inverted_index.bin"  # 倒排索引二进制文件
INVERTED_INDEX_COMPRESSED_FILE = "inverted_index_compressed.bin"  # VB压缩倒排索引文件
DICTIONARY_FILE = "dictionary.txt"  # 词典文件名（文本格式）
DICTIONARY_BIN_FILE = "dictionary.bin"  # 词典文件名（二进制格式）
...
# 文档ID和位置存储参数
DOC_ID_SIZE = 4  # 文档ID存储大小（字节）- 32位无符号整数
POSITION_SIZE = 2  # 位置存储大小（字节）- 16位无符号整数（最大65535）
FREQ_SIZE = 4  # 词频存储大小（字节）
POSITION_MAX_VALUE = (1 << 16) - 1  # 位置最大值 (65535)
...
def generate_compressed_dictionary(dict_text_path: str, compressed_index_path: str, output_dir: str) -> str:
    ...
    # 词典压缩参数配置
    TERM_BYTES = 8          # 每个词项前缀固定 8 字节
    FREQ_BYTES = 4          # 文档频率字段，4 字节
    PTR_BYTES = 4           # 倒排表指针字段，4 字节
    TERM_PTR_BYTES = 3      # 词项指针字段，3 字节
    TERM_LEN_BYTES = 1      # 词项长度字段，1 字节
    K = 4                   # 每4个词项中，只有1个存储完整指针
    
    TERM_ENTRY_WITH_PTR_SIZE = TERM_BYTES + TERM_PTR_BYTES + FREQ_BYTES + PTR_BYTES  # 19字节（带指针）
    TERM_ENTRY_WITH_LEN_SIZE = TERM_BYTES + TERM_LEN_BYTES + FREQ_BYTES + PTR_BYTES  # 17字节（带长度）
