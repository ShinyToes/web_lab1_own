...
class InvertedIndexBuilderWithSkip(InvertedIndexBuilder):
    ...
    def __init__(self, config_path: str = None):
        ...
        # 读取跳表指针步长策略
        self.skip_strategy = skip_config.get("skip_strategy", "sqrt")
        self.skip_step = skip_config.get("skip_step", None)  # None表示使用策略计算
        ...
        if not self.skip_enabled:
            ...
            print(f"  - 步长策略: {self.skip_strategy}")
            if self.skip_step is not None:
                print(f"  - 固定步长: {self.skip_step}")
    
    def _calculate_skip_step(self, list_length: int) -> int:
        """
        根据配置的策略计算跳表步长
        ...
        返回:
            跳表步长
        """
        if list_length < self.min_list_length:
            return 1
        ...
        elif self.skip_strategy == "sqrt":
            # 平方根策略：√L（默认，最优平衡）
            skip_step = int(math.sqrt(list_length))
        elif self.skip_strategy == "log":
            # 对数策略：log2(L)（更密集的指针）
            skip_step = int(math.log2(list_length)) if list_length > 1 else 1
        elif self.skip_strategy == "linear":
            # 线性策略：L/k（k默认为10）
            linear_factor = getattr(self, 'skip_linear_factor', 10)
            skip_step = max(1, list_length // linear_factor)
        elif self.skip_strategy == "cbrt":
            # 立方根策略：∛L（更稀疏的指针）
            skip_step = int(list_length ** (1/3))
        else:
            # 默认使用平方根策略
            skip_step = int(math.sqrt(list_length))
        
        return max(1, skip_step)
    ...
    def calculate_skip_pointers(self, doc_ids: List[int], file_positions: List[int] = None) -> List[Tuple[int, int, int]]:
        ...
        支持的步长策略：
        - "sqrt": 使用√L作为步长（默认，最优平衡）
        - "log": 使用log2(L)作为步长（更密集的指针）
        - "fixed": 使用固定的skip_step值
        - "linear": 使用L/k作为步长（k为固定因子，默认10）
        ...
        # 使用辅助方法计算跳表步长
        skip_step = self._calculate_skip_step(L)
    ...
    def generate_binary_inverted_index(self, inverted_index_path: str) -> str:
        ...
        with open(output_path, 'wb') as f:
            ...
            for term in sorted(inverted_index.keys()):
                ...
                skip_step = self._calculate_skip_step(len(doc_ids_int))
                skip_positions = []
